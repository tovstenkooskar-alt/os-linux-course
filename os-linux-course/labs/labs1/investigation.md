**6. Часть С. Проверить данные двумя способами**

| Характеристика | Способ 1 | Способ 2 | Совпало? / комментарий |
| :--- | :--- | :--- | :--- |
| Версия Windows | winver | msinfo32 / Параметры | **Совпало.** И утилита winver, и окно «Параметры» показывают идентичные данные: Windows 10 Pro, версия 22H2. *(Примечание: так как 1 часть задания я делал на ПК в колледже, эти значения могут отличаться от характеристик в начале отчета).* |
| Модель процессора | Параметры / msinfo32 | Диспетчер задач → CPU | **Совпало.** Диспетчер задач во вкладке Производительность подтверждает данные c msinfo32 установлен процессор Intel(R) Core(TM) i7-10700KF CPU @ 3.80GHz. *(Примечание: так как 1 часть задания я делал на ПК в колледже, эти значения могут отличаться от характеристик в начале отчета).* |
| Объём RAM | Параметры / msinfo32 | Диспетчер задач → Memory | **Совпало.** Диспетчер задач фиксирует общий объем оперативной памяти 32ГБ, что совпадает с msinfo32. *(Примечание: так как 1 часть задания я делал на ПК в колледже, эти значения могут отличаться от характеристик в начале отчета).* |

![О системе](../../assets/screenshots/02_about.png)

![Память](../../assets/screenshots/04_memory.png)

![Процессы(доказательство много задачности)](../../assets/screenshots/07_processes.png)

![Сведенья из msinfo32](../../assets/screenshots/08_msinfo32.png)

| **Процессы** | 
1. Процесс 1: happ, vpn, 3% cpu/345mb ram
2. Процесс 2: edge, браузер, 3% cpu/321mb ram
3. Процесс 3: windor defender, защита os, 1% cpu/143mb ram |

**Короткие ответы на вопросы:**

* **Что относится к пользовательскому пространству:** К нему относятся все прикладные программы (браузеры, мессенджеры, среды разработки вроде Visual Studio Code), графический интерфейс пользователя, а также библиотеки и фоновые службы. Все они работают с ограниченными правами и не имеют прямого доступа к аппаратному обеспечению.
* **Зачем ОС управляет CPU, RAM, диском и устройствами:** Чтобы обеспечить стабильность, безопасность и  распределение ресурсов между программами. ОС изолирует процессы друг от друга чтобы одна программа не стерла данные другой в оперативной памяти, предотвращает зависание всей системы из-за одного приложения и предоставляет программам удобный универсальный интерфейс для работы с любыми устройствами.
* **Где наблюдается работа ОС как посредника:** Самый наглядный пример — Диспетчер задач, где видно, как ОС распределяет CPU и RAM между запущенными процессами.